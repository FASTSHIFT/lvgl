        ...
        for(uint16_t i = 0; i < scale; i++) {
            if(barcode->direction == LV_DIR_HOR) {
                ...
                if(barcode->tiled) {
                    ...
                    lv_memset(dest, lv_color_to_int(color) ? 0xFF : 0x00, draw_buf->header.stride);
                    dest += draw_buf->header.stride;
                }
            }
        }
    }

    if(!barcode->tiled && barcode->direction == LV_DIR_HOR) {
        int32_t h = buf_h - 1;
        uint8_t * dest = src + draw_buf->header.stride;
        while(h--) {
            lv_memcpy(dest, src, draw_buf->header.stride);
            dest += draw_buf->header.stride;
        }
    }
